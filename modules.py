...
class Gnss_module:
    ...
    def calculate_projection(gnss_data, altitude):
        ...
        for time_s, x_mm, y_mm, roll_deg, pitch_deg in gnss_data:
            try:
                ...
                rotation_matrix = nv.angle2dcm(yaw, -pitch_deg, roll_deg, input_unit=angle_unit, rotation_sequence='ZYX')
                transformed_vector = list(np.dot(np.transpose(rotation_matrix), vec))
    ...
    def calculate_heading(gnss_data):
        ...
        try:
          ...
          headings = np.arctan2(delta_ymm, delta_xmm) * 180 / np.pi
